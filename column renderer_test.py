...
def note_load(note_file,bpm,screen,clock,note_current,rect_note_current,sp,start_time):
	...
	sp = note_judge(note_file,note_current,rect_note_current,current_beat,sp,length,screen) #文件指针，用于查询note的信息
	...
	while i < len(note_current): #判断是不是读完了note
		...
		note_beat = note_info[0]*bar_delta + note_info[1]/note_info[2] #计算拍数x+y/z
...
def note_judge(note_file,note_current,rect_note_current,current_beat,sp,length,screen):
	while(sp < length):
