...
def note_load(note_file,bpm,screen,clock):
	...
	while(sp != -1):
		...
		note_wait = list();
		sp = note_judge(note_file,note_wait,bar_count,sp,length)
		for i in range(0,len(note_wait),1):
			...
			note_draw(column,screen)
			clock.tick(bar_delta*note_bar_div)
...
def note_draw(column,screen):
	s_width = pg.Surface.get_width(screen)
	s_height = pg.Surface.get_height(screen)
	rect = pg.Rect(0,0,50,10)
	if(column == 0):
		rect.center = (s_width/2-3*25,0)
		while(rect.y > s_height):
			rect.y += s_height/10
			pygame.draw.rect(screen,'white',rect, 0)
	elif(column == 1):
		rect.center = (s_width/2-1*25,0)
		while(rect.y > s_height):
			rect.y += s_height/10
			pygame.draw.rect(screen,'white',rect, 0)
	elif(column == 2):
